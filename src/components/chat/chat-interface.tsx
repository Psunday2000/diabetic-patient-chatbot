...
export default function ChatInterface({ messages, onSendMessageSubmit, isLoading }: ChatInterfaceProps) {
  ...
  useEffect(() => {
    ...
    // This will only trigger when the messages array changes
    scrollToBottom();
  }, [currentMessages]);
  ...
  return (
  <div className="flex flex-col flex-1 bg-card h-full min-w-0 pt-16">
    <header className="bg-secondary/30 px-4 py-3 border-b border-border">
          ...
          {currentMessages.map((msg) => (
           ...
           {isLoading && (
            <div className="flex justify-start">
              <div className="flex items-end max-w-full md:max-w-[65ch] flex-row w-full">
                <div className="flex-shrink-0 mr-3 self-start mt-1">
                  <div className="h-8 w-8 rounded-full bg-primary/10 flex items-center justify-center">
                    <Activity className="text-primary h-5 w-5" />
                  </div>
                </div>
                <div className="p-3 rounded-xl shadow-sm bg-muted text-muted-foreground rounded-bl-none max-w-full">
  ...
  <div className="p-4 border-t border-border bg-background shadow-sm">
        ...
        <form onSubmit={handleSubmitForm} className="flex items-end space-x-3">
           <Textarea
            ...
            disabled={isLoading}
            rows={1}
            className="flex-1 rounded-lg px-4 py-2 focus-visible:ring-offset-0 resize-none max-h-32 bg-background/40 text-foreground"
